...
**下一步工作（按序）**：
1. **Task 10 收口**——runbook Phase 8 章节、敏感扫描、独立复审、最终 disposable 矩阵与台账。
2. **独立事项（Phase 8 完成后）**：①迁移 26 上生产（需显式授权 + 备份演练沿用 §5 规程）；②生产 web 应用部署（当前无持久 web 进程，nginx 仅静态站）；③Phase 8 分支合并回主线（沿用 7B 双父 merge 惯例）。
