...
| 3 | Reference Ledger migration + protected commands + RLS + 安全联动 SQL | 🔨 迁移已写完（1795 行 / 32 对象），待 014 pgTAP 与 disposable 验证 | 迁移内容：URL/域名归一化函数（SQL 与 TS 同规则）、6 张表（2 canonical 当前行 + 2 决策历史 + `reference_security_flags` + `reference_review_commands` 回执）、4 个受保护命令（`submit_register_domain_authority` / `submit_decide_domain_authority` / `submit_register_reference` / `submit_decide_reference`）、3 个视图（1 内部 + 2 公共）、RLS（anon 仅可读 verified 引用的 5 个公开列）、安全联动触发器（`security_indicators` INSERT 后匹配并插入 flag）、outbox 白名单扩展（4 个新事件 + 完整保留 7A 既有 payload 校验分支）。错误码 AR201/202/203/204/206/207/208/209/210/299。结构自检：17 函数全部 `owner to postgres`、美元引号 17/17 配对、AR 码与契约枚举一致。**编写期避免的两个缺陷**：① 重写 outbox payload 约束时曾把 7A 对 ai_run/security 事件的校验分支整个丢掉，已重建为「7A 全部分支 + reference 分支」的正确并集；② `occurredAt` 直接传 timestamptz 会产生 `…+00:00` 格式、被自己的约束拒绝，已改为 7A 同款 `to_char(… at time zone 'UTC', '…MS"Z"')`。**尚未跑 014 pgTAP / disposable reset / typegen**（已获授权，下一步执行）。 |
