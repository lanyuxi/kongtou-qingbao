...
> 最近更新：2026-09-01（**Phase 7B 全部 10 个任务完成，最终验收清单 14/14 闭环**；含用户授权的 disposable 矩阵：full pgTAP 1,501 + integration 74。Phase 7B 已完结、未部署。）
> 权威规范：仓库根目录 `AGENTS.md`（产品规则与工程约束的唯一事实来源，本手册不重复其内容，只补充现状与经验）
>
> **当前一句话状态（2026-09-01 Phase 7B 完结）**：Phase0–7A 全部在主线 `codex/phase-0-1-foundation`；**Phase7B Task1–10 全部完成**，实施在独立分支 `codex/phase-7b-references` / worktree `.worktrees/phase-7b-references`（**未合并主线**，HEAD `2a63309`）。七步全闭环：Golden Dataset（6 用例/10 测试，变异 5/5 killed）、runbook Phase 7B 章节（10 个 `AR2xx` 码经迁移源码核实）、敏感扫描零命中、独立复审两轮闭环（最终 **SPEC COMPLIANT / APPROVED / 0C / 0I / READY YES**，修掉 `%` 与冒号两处方向相反的跨层归一化分叉、Golden Dataset 断言区分力、`referenceUrlSchema` 重复声明、AR202/AR203 修正）、**disposable 矩阵（用户授权「确认授权」）**：预检（7 文件哈希字节一致）→ 一次 reset → full pgTAP **16 files / 1,501 tests PASS** → 临时 IPv4 双隧道 + 0600 env（值只写文件不回显）+ 三端点一致性 PASS（均=2）→ integration **11 files / 74 tests PASS**（含 reference-review-repository 与 reference-security-coupling）→ 残留核验全零 → 隧道关闭/临时文件删除/远端复核；最终验收清单 **14/14**。全仓 `pnpm verify` exit 0 = contracts 171 + domain 412 + database 302 + worker 246 + web 354 = **1,485 non-skipped / 76 gated skips**。**生产未访问（54321/54322 全程未触碰），生产仍为 19 个已应用迁移，第 20–25 个按 production unapplied 处理。**

**2026-09-01 Task 9 public link resolution + unverified-link marking COMPLETE**（提交 `8823eca`）：公开项目详情页的对外链接从此只来自已验证白名单引用。新增 `apps/web/src/components/reference-elements.tsx`（126 行）四个导出：①`resolveOfficialWebsite`——`official_website_url` 与引用列表的匹配必须经 `@airdrop/domain` 的 `normalizeReferenceUrl`（与登记时同一纯函数），一个值不可能「以形式 A 通过验证、以形式 B 渲染」；归一化失败的 URL（如 `javascript:`）永不匹配；`www.` 与裸域名按产品决策仍是独立权威，不会静默互认。②`OfficialWebsiteLink`——blocked 时惰性文本「已停用」，未验证时惰性文本「未验证」+ 原始线索（可见不可点），匹配已验证引用时才渲染锚点（`rel="noreferrer noopener"`、`target="_blank"`）。③`VerifiedReferenceLinks`——锚点 + `<time dateTime={lastVerifiedAt}>`（核验时间机器可读）。④`GrantedDomainChips`——已授权域名只作所有权芯片，永不为链接。`project-detail-loader` 增加第三参 `referenceRepository`，六个读取一次 `Promise.all` 组合，引用快照不因更慢的评分读取而改变（有独立测试）。旧 `security-elements` 的 `OfficialWebsiteLink` 与 3 个被取代测试已移除，其「公开组件源码不得含内部字段」卫生测试独立成块并纳入 `reference-elements.tsx` 扫描。变异检验 **8/8 killed**。**计划外改动已记录**：`apps/web` 新增 `@airdrop/domain` workspace 依赖（lockfile 仅加内部链接）与 `globals.css` 的 `.reference-*` 样式（全局 `a` 规则会剥离链接外观）。

**⚠️ `@airdrop/domain` 的客户端边界（接手者必读）**：该包入口 `index.ts:127` 重导出了 `intelligence/evidence-grounding.js`，而后者 `import { createHash } from 'node:crypto'`——**从 `@airdrop/domain` 根导入任何东西都会传递性引入 Node 内置模块**。目前安全（`reference-elements.tsx` 只被服务端组件 `page.tsx` 引用），但只要有人从 `'use client'` 组件引用它，浏览器 bundle 就会因解析不到 `node:crypto` 而构建失败，且该陷阱在 import 处完全不可见。已加回归测试：`reference-elements.test.ts` 的 "domain package client boundary" 扫描 `apps/web/src` 所有 `'use client'` 模块，断言无一处导入 `@airdrop/domain`（变异验证：临时造一个引用它的客户端组件，测试精确报出文件名）。将来若需在客户端做归一化，正确做法是为 domain 包加 `./references` 子路径导出以绕开 `node:crypto`，而非放宽该断言。

**2026-09-01 Task 10 Step1–2 Golden Dataset COMPLETE**（提交 `fb6ddb6`）：新增 `apps/worker/src/ai/fixtures/reference-golden.ts`（6 用例）与 `apps/worker/src/ai/tests/reference-golden.test.ts`（10 测试，worker +10）。用例覆盖计划点名五类——明确官方公告、社交账号冒充、近似域名（`nova-airdrop.example` vs 官方文档的 `nova-airdrop.io`）、提示注入、虚构 Evidence 定位符——另加越权字段注入。抽取契约没有引用专属声明类型，按计划文件清单（不含 `extraction.ts`）不新增枚举，改用 `other_intelligence` + `signalType: 'official_reference_proposal'`（signalType 本就是自由 snake_case）。核心断言：任何提议只能成为 `other_intelligence` 候选，`canonicalReferenceCount` 恒为 0，且载荷不得含 13 个 canonical 台账键。**变异检验 5/5 killed**：三项 fixture 级变异证明负例非空转（虚构定位符改成真实引文、注入文本作为引文、移除越权字段），两项实现级变异证明真正的防御是 grounding 与严格 schema（`filterGroundedCandidates` 的 `includes` 改恒真、`.strict()` 改 `.passthrough()`）。

**2026-09-01 Task 10 Step4–5 runbook + 敏感字段 review COMPLETE**（提交 `d6f6aac`）：`docs/runbooks/local-development.md` 新增 Phase 7B 章节，沿用 Phase 7A 七子节结构，含 10 个 `AR2xx` 稳定错误码。**错误码语义逐条对照迁移源码核实并修正两处易错点**：`AR202` 是「状态 × 决策」转移矩阵不允许（如重复 verify 已验证的引用），`AR203` 才是「无 `granted` 权威覆盖该 URL 的 host」——二者不可混用；`reference_flag_already_released` 与 `reference_command_invalid` 共用 `AR208`。Step5 敏感字段扫描在 `apps/web` + `contracts/src/references` + `database/src/references` 范围内零命中，并以范围外的既有命中反向验证扫描模式有效、排除假阴性。`pnpm verify` / `git diff --check` / 占位符检查均 exit 0。

**2026-09-01 Task 10 Step6 独立复审 → APPROVED**（提交 `66dec53`、`3863e9f`）：首轮复审区间 `17fd364..dd3675c` 结论 **0 Critical / 2 Important / 4 Minor**；修复后定向复审判定 **SPEC COMPLIANT / APPROVED / 0C / 0I / READY YES**，两条 Important 均 ADDRESSED（复审者实测复现 RED 与三项变异，其中最有力的「两用例整对互换」变异修复前全绿、修复后稳定失败）。四项核心安全属性**全部核实成立且有测试锁定**：未验证 URL 不可能成锚点、blocked 不渲染引用、域名芯片非链接、敌意 label 惰性。

**本次最有价值的发现是两处方向相反的跨层归一化分叉**（都在引入 `@airdrop/domain` 依赖、让 `resolveOfficialWebsite` 真正跑起来之后才被生产路径激活）：①**`%` 检查 TS 过严**——SQL 只查 authority 段（规格要求拒绝的是「% 编码 **host**」），TS 却拒绝任何位置的 `%`，导致 `https://example.com/a%20b` 这类 URL 可被台账接受并完成 verify，却在页头被渲染成「未验证」；②**冒号检查 TS 过松**——SQL 统计整个 `v_rest` 的冒号数，TS 只统计 authority，于是 `https://example.com/time?t=12:30:45` 被 TS 错误接受（台账永远不会存它）。两处均已对齐 SQL 并有 RED 用例坐实。**教训：跨层归一化一致性绝不能靠「看代码觉得一样」，必须逐条对拍；同一个文件里能同时藏着方向相反的两处分叉。**

复审另一条 N-2 也已修（提交 `31f58f3`）：`referenceUrlSchema` 原在 `commands.ts:32` 与 `projections.ts:16` **各有一份重复声明而非共用**，且投影那份缺控制字符 refine——实测公开投影**会接受**带控制字符的 URL（RED 实证 `expected true to be false`），而控制字符正是注入值用来逃出 `href` 的手段。已改为 `projections.ts` 从 `./commands.js` 导入共享 schema（无循环依赖），并补 RED 用例；变异检验把共享 schema 的控制字符 refine 改为恒真后，新增投影用例与既有命令用例**同时失败**。**教训：同一约束的两份重复字面量「碰巧一致」不算一致，只有共享声明才能保证不漂移。**

仅剩一条 Minor 后续项（无需授权、fail-closed）：SQL 的 `normalize_reference_url_v1` 缺长度检查——注意我最初把它误判为「不可达」，已在 `3863e9f` 更正；空路径归一化补 `/` 会 +1 字符，2048 输入可存成 2049 并在读取时触发投影 `.max(2048)` 抛错、影响整页引用列表。要彻底消除需新增 forward-only 迁移，属 Task 3 冻结迁移之外的独立决策。

**2026-09-01 Task 10 剩余项 → 已全部闭环**：Step3 已在用户授权下完成（见上方 Step3 段落与 worktree 内权威记录）；两处 Modify 均已处理——`projections.test.ts` 借 N-2 修复补了控制字符 RED 用例，`reference-review-repository.integration.test.ts` 则在授权后的 integration 矩阵中实际运行通过（11 files / 74 tests）。Phase 7B 无剩余待办；后续为「合并主线」与「生产 rollout（仍被 reviewer 供给/Auth 健康/备份演练/显式授权四前置阻塞）」两个独立决策。

**2026-09-01 Task 10 Step7 最终台账**：运行时 Node `v22.22.2` / pnpm `11.16.0`；`pnpm verify` exit 0，全仓 **1,485 non-skipped / 76 gated skips**（contracts 171 + domain 412 + database 302 + worker 246 + web 354）；`git diff --check` 与占位符检查 exit 0。**哈希逐个复算零漂移**：migration23 1,994 行 `75e038d8…`、migration24 1,241 行 `570e6b89…`、migration25 591 行 `07665b57…`；pgTAP 014 956 行 `31cee8dc…`、015 957 行 `a34cced6…`、016 292 行 `0fa44939…`；generated types 158,662 bytes `e5dc3861…`。**最终验收清单 14 项已分类**：✅ **本地已验证 7 项**（②未验证链接不渲染、⑤`last_verified_at` 随链接展示、⑦blocked 不渲染引用、⑧AI 不能产生 canonical 核验、⑪公开投影无泄漏、⑫UI 惰性渲染无泄漏、⑭文档区分本地完成与生产部署）；🟡 **部分 1 项**（⑬Golden Dataset / Web build / `pnpm verify` 已过，full pgTAP 与 integration 待 Step3）；⚠️ **需 Step3 数据库证据 6 项**（①五个台账对象与受保护命令/RLS/追加式防护/回执/审计/outbox、③已验证域名不验证其下 URL、④Evidence→Raw Item→Source 追溯、⑥indicator 同步移除与人工恢复、⑨活跃角色要求与 fail-closed、⑩共享目标锁竞态顺序）。**关键提醒：本地门禁全绿不能当作数据库行为的证据**——后 6 项只存在于迁移与 pgTAP 中，必须 Step3 才能闭环。生产未访问，仍为 19 个已应用迁移。

**2026-09-01 Task 8 reviewer workflow COMPLETE**：四组件 + 三页面。`authority-list.tsx`（146 行）：域名权威列表、状态过滤、游标、`authorityDetailHref`。`authority-detail.tsx`（328 行）：权威详情与不可变历史、决策表单（grant/revoke/regrant），并承载共享的 `ReferenceSubmissionState` 与 `SubmissionMessage`。`reference-list.tsx`（271 行）：引用列表、状态过滤、游标、`referenceDetailHref`，以及候选引用登记表单。`reference-detail.tsx`（329 行）：引用详情（含域名权威上下文、最近核验时间、活跃安全指标）与决策表单（verify/reverify/restore/withdraw）。页面：`/review/references`（权威 + 引用双列表 + 登记表单）、`/review/references/[referenceId]`、`/review/references/authorities/[authorityId]`。

**Task 8 五条硬约束**：①**内部链接只由 UUID 校验过的 id 生成**——`authorityDetailHref`/`referenceDetailHref` 用 `uuidPattern` 校验，不过则退为惰性文本，敌意 id 永远变不成可点链接；②**高风险确认绑定 `{id, version}` 快照**——同时比对 draft 快照与 `{id, version}`，任一变化即失效，`update()` 与 409 都会清除确认；③**verify/reverify/restore 必须有 Evidence**（由 contract schema 的 refine 强制，非 UI 层判断），withdraw 不需要；④**note 与自由文本一律经 `displayReviewValue`**；⑤所有决策都绑定界面上显示的 aggregate version 作为 `expectedVersion`。

**2026-09-01 Task 8 变异检验 19/19 killed（首轮 14/17，三项存活后补测）**：三项存活都指向同一个测试盲区——**React 会自动转义文本节点，所以只断言 `not.toContain('<script>')` 无法区分「被转义」与「被隐藏」**，`displayReviewValue` 这类防护等于没被覆盖。补 `not.toContain('&lt;script&gt;')` / `&lt;a href` 后 A6/B6 被杀。C3（引用侧「未确认不得提交」）是漏写用例，已补。另注意：B6 首轮存活还因为变异打在 `authority-detail.tsx` 的 `Metadata`，而敌意测试渲染的是 `ReferenceDetail`（两文件各有自己的 `Metadata`），已补 `AuthorityDetail` 敌意用例。**接手者注意：四个 reference 组件各自持有私有的 `Field`/`Text`/`Metadata`，与安全模块先例一致；改一个不会联动其他。**

**2026-09-01 Task 8 计划外改动（已记录待确认）**：`review-browser-runtime.ts` 增加 `referenceApi` 字段。计划的 Task 8 文件清单只列了 `review-shell.tsx`，但页面需要客户端入口，无此字段则页面拿不到 `ReferenceReviewApiClient`。这是必要连带改动。此外 Task 7 遗留的两个 detail GET 路由已在本轮开头补齐（提交 `c016d9c`）。

**2026-09-01 Task 7 BFF 实现 COMPLETE**：三个模块 + 五个路由。`reference-cursor.ts`（34 行）在页面边界用 `safeParse` 校验三种不透明游标，畸形值一律回落到首页而非抛服务端错误，且三种游标互不可串解。`reference-review-handlers.ts`（454 行）导出 9 个工厂：引用侧 list/detail/register/decide、权威侧 list/detail/register/decide、以及公开的 `createPublicReferencesHandler`。三条硬约束：①**认证严格先于查询与请求体校验**，401 时仓库零调用；②畸形游标单独映射 `invalid_cursor`，其余校验失败一律 `invalid_request`；③查询类失败回落 `reference_query_failed`，变更类回落 `reference_persistence_failed`，二者不可混用。`decide` 处理器额外要求请求体 id 与路径参数逐字相等。`reference-review-api-client.ts`（247 行）每次调用重新取 token、每次变更重新生成幂等键，409 报为类型化 `conflict` 且**从不重试**，入参在发出请求前用 contract schema 校验，不合法则零网络调用。路由：`/api/v1/review/references`（GET+POST）、`/api/v1/review/references/[referenceId]/decisions`（POST）、`/api/v1/review/references/authorities`（GET+POST）、`/api/v1/review/references/authorities/[authorityId]/decisions`（POST）、`/api/v1/references`（公开 GET，无认证）。

**2026-09-01 Task 7 两处继承测试缺陷修正（接手者必读）**：本轮接手时 worktree 内已有 3 个未跟踪的 Task 7 RED 测试（14:46–14:50 写入，非本会话产出），直接以其为契约实现，但其中两处断言自相矛盾、无论如何实现都不可能通过，已按 security 先例修正：①`requires a valid idempotency key for %s` 原复用带有效 key（`'reference-command-1'`）的 `mutationRequest`，与同文件「routes register commands once」用例直接冲突——已新增 `invalidMutationRequest`（空白 key `' '`），与 `security-review-handlers.test.ts` 的双助手结构一致；②`missing project scope` 行的 `suffix` 逻辑在 query 为空时仍拼出 `?projectId=X`（合法请求，应返回 200），已拆成独立的无 query string 用例，并把原表补一行 `malformed project scope`。另修 `noUncheckedIndexedAccess` 下 `const [first, second] = keys` 的 TS2464 与未使用的 `authorityId` 常量。**提醒：该 worktree 存在并行写入者，接手前务必先 `git status` 确认未跟踪文件归属。**

**2026-09-01 Task 7 变异检验 16/16 killed**：handlers 8 项（去 401 门、409 降为 400、去路径/请求体 id 绑定、跳过幂等键校验、接受非 UUID 路径参数、畸形游标映射为 invalid_request、公开读取放宽 projectId、查询类 verifier 失败改用持久化错误码）；client 6 项（409 重试一次、null 查询值当文本发出、token 全客户端复用、非法 command 仍发网络、去路径/命令 id 一致性、幂等键复用）；cursor 2 项（不校验直接透传、undefined 当作有效游标）。**C3 首次变异因只构造未使用的 `cachedDeps` 而空转存活**，已重写为在 `request()` 内真实缓存 token 后 killed。打包隔离：`review-client-bundle.test.ts` 已纳入 `reference-review-api-client.ts`，新增 `referenceApiRuntime` 标记断言与 `createReferenceReviewRepository` 禁用断言。

**2026-09-01 Task 7 未完成项**：实施计划的 Task 7 文件清单**没有引用/权威的 detail GET 路由**，`createReferenceDetailHandler` 与 `createDomainAuthorityDetailHandler` 因此已实现并测试但**尚无路由挂载**。Task 8 的详情页需要它们。本轮按批准的文件清单执行、未擅自加路由；若要在 Task 8 之前补齐，需新增 `app/api/v1/review/references/[referenceId]/route.ts` 与 `app/api/v1/review/references/authorities/[authorityId]/route.ts`（注意 `[authorityId]` 目录下相对导入为 8 层 `../../../../../../../../lib/`，比 `[referenceId]` 的 7 层多一层）。
...
**2026-08-31 Task 5 SDD 1A Step 2 integration RED artifact COMPLETE**：在 linked worktree 新增环境门控 `packages/database/src/tests/reference-security-coupling.integration.test.ts`，含五个具名真实路径：同 reference key 的 flag-first/verify-first、同 authority key 的 revoke-first/verify-first、Evidence-required restore/history、source-key wait 后 block-wins、exact cleanup/zero-residue。套件使用 `owner`/`blocker`/`inserter` 三个 `max:1` PostgreSQL 连接、fresh Supabase auth clients 与真实 `ReferenceReviewRepository`；blocker 仅为观察屏障，生产 trigger/RPC 必须自己成为同 key waiter。未提供四个 integration 变量，故未执行数据库行为。

**2026-08-31 Task 5 SDD 1A Step 3 local gated RED checks COMPLETE**：linked worktree `test:integration` 显式追加新文件；Node 22.22.2 直接入口 database eslint 与 `tsc --noEmit` 均 exit 0。focused Vitest 为 **1 file skipped / 5 tests skipped / exit 0**，确认环境门控而非行为通过。integration SHA-256 为 `4679ebd8df2de37debc49b75dd46a0a3430ce6fc34ff6b23896456268a7212aa`。真正 behavioral RED 仍未运行，必须先获得仅 disposable 的明确授权；未连接数据库、远端或生产。

**2026-08-31 Task 5 SDD 1A Step 4 pgTAP 016 structural RED artifact COMPLETE**：linked worktree 新增 `supabase/tests/016_phase_7b_reference_security_locking.test.sql`，`begin; plan(18); finish(); rollback;` 完整，含 trigger/reference/authority lock order 与 post-lock re-check、security-definer/owner/signature/privilege/direct-ledger denial 断言。016 SHA-256 为 `6d26ad0acecc7ebd9f69b6daec0d821cf717896365a3b366f0a676cfb4151ada`；`git diff --check` exit 0。函数定义断言仅为结构补充，load-bearing behavioral RED 仍待 disposable 授权；migration25 未创建，第23/24 SHA 仍为 `75e038d8…5966` / `570e6b89…aad8`。
...
**2026-08-30 Task 4 修复版 corrected focused RED 预检**：用户授权后重复 fail-closed 预检 exit 0。本地 015 为 957 行 / SHA `a34cced69ca69a5247f0ff041851af2cdda47d61bf916098383fb29639e5dd5b`，远端仍为旧 SHA `83a15ab336a0ab4a45f7ac270f15fb38a18a8a201ea349f4ced7f3f6c9770750`；exact workdir/project、DB/Kong running+healthy、唯一端口 64322/64321、24 migrations / 最高 `20260830000100`、paused marker 与 seed/23/24/014 SHA 全匹配。下一步只覆盖 disposable 同名 015 并核 SHA/行数；不 reset、不复制 migration、不建隧道、不访问生产。

**2026-08-30 Task 4 修复版 corrected 015 同步**：仅覆盖 disposable 同名 test，copy exit 0；独立 SSH 核验为 957 行 / SHA `a34cced69ca69a5247f0ff041851af2cdda47d61bf916098383fb29639e5dd5b`，与本地完全一致。未复制其他文件、未 reset、未建隧道、未访问生产；下一步只运行 focused 015 RED。

**2026-08-30 Task 4 修复版 corrected focused RED 完整成立**：focused 015 为 **Files=1 / Tests=59 / Failed=10 / exit 1**；Test 10–13、39–44 分别精确证明四 RPC OUT shape、nested authority 命名及四个 aggregate version 值缺口，其余 49 项 PASS。无 SQL/TAP/fixture 噪声；未 reset、未复制 migration、未 full/typegen、未访问生产。下一步仅本地最小修复 production-unapplied 第24 migration；远端 GREEN 需新授权。

**2026-08-30 Task 4 aggregate-version migration 本地最小实现**：仅在第24 migration 的四个 reviewer read RPC 增加独立 `referenceVersion` / `authorityVersion` 并返回 canonical row version，nested authority 把旧 `version` key 改为 `authorityVersion`；顶层 protocol `version` 仍为 literal 1。migration 现 1,241 行 / SHA `570e6b89a7bbb2b48c1356fd0b210b741a46bc64ee4d4283a54431c9b509aad8`，015 未改，static alignment 与 diff check 通过。尚未同步/reset/typegen/访问生产；下一步本地窄门禁，disposable GREEN 需新授权。

**2026-08-30 Task 4 aggregate-version 本地 GREEN**：contracts lint/typecheck exit 0、16 files / 167 PASS；database lint/typecheck exit 0、focused repository/entrypoints 34 PASS / 3 integration gated。未安装依赖、未连接数据库；generated types 保持旧 schema 产物，等待新 schema 的双次稳定 typegen。下一步需新授权 disposable 同步修改后第24 migration、reset、focused/full pgTAP、双 typegen、generated types 更新与 repository integration/清理；生产不访问。

**2026-08-30 Task 4 aggregate-version disposable GREEN 预检**：用户授权后复核隔离 worktree/branch，并完成只读 fail-closed gate。远端 exact workdir/project、CLI 2.112.0、DB/Kong healthy、唯一端口 64322/64321、24 migrations/最高第24、paused marker 与 seed/23/014/015 SHA 全匹配；远端第24为旧 SHA `04a912f9…561faa` / 1,233 行，本地新版为 `570e6b89…aad8` / 1,241 行；本地 16432/16433 关闭。下一步仅同步新版第24并核字节，之后才 reset；生产未访问。

**2026-08-30 Task 4 aggregate-version 第24 migration 同步**：仅同步该 migration，远端独立核验为 1,241 行 / SHA `570e6b89a7bbb2b48c1356fd0b210b741a46bc64ee4d4283a54431c9b509aad8`，与本地完全一致。未复制其他文件、尚未 reset；下一步执行授权的 disposable reset，失败则停止后续链。生产未访问。

**2026-08-30 Task 4 aggregate-version disposable reset GREEN**：`db reset --yes` 完成，第1–24 migration 依序应用，修改后的第24 migration 真实编译/执行通过，seed/容器重启成功。reset 后 DB/Kong healthy，migration count 24 / max `20260830000100`，paused marker count 1，第24 SHA 保持 `570e6b89…aad8`。下一步 focused 015，失败即停止；生产未访问。

**2026-08-30 Task 4 aggregate-version focused 015 GREEN**：1 file / 59 tests / PASS / exit 0，corrected RED 的 10 个 aggregate-version/naming 缺口与原有 49 个断言全部闭环，无 SQL/TAP 噪声。下一步全量 pgTAP，失败即停止 typegen/integration；生产未访问。

**2026-08-30 Task 4 aggregate-version 全量 pgTAP GREEN**：15 files / 1,483 tests / PASS / exit 0，001–015 全部通过；新增 6 个 corrected aggregate-version 断言已进入完整矩阵，既有行为无回归。下一步同 schema 双次 typegen 并逐字节比较，仅稳定一致才更新本地 generated types；生产未访问。

**2026-08-30 Task 4 aggregate-version 双次 typegen 稳定**：同一 disposable schema、pinned CLI 2.112.0 连续生成两份，均为 158,662 bytes / SHA `e5dc3861ebab1eda8206f7fca1a23d760d7c5a75b01b9ff3732bd734f17aedef`，逐字节一致。下一步仅下载第一份到本地临时路径并复核，确认后机械替换 generated types，最终清理远端临时目录；生产未访问。

**2026-08-30 Task 4 aggregate-version typegen 下载复核**：第一份稳定产物下载至本地临时路径，复核仍为 158,662 bytes / SHA `e5dc3861…aedef`；四个 reviewer RPC 的生成类型均含正确 aggregate 字段。当前仓库 generated types 为旧 158,522 bytes / SHA `24733d6d…cf11`，预期不同。下一步仅机械替换并 `cmp` 证明一致。

**2026-08-30 Task 4 aggregate-version generated types 更新**：已机械替换仓库 generated 文件，`cmp` 证明与稳定产物逐字节一致；最终 158,662 bytes / SHA `e5dc3861…aedef`，累计较 Task 3 基线新增 77 行，diff check 通过。下一步 database 全包 lint/typecheck/test；生产未访问。

**2026-08-30 Task 4 aggregate-version database 本地全包 GREEN**：lint/typecheck exit 0，Vitest 14 files / 286 PASS，10 integration files / 69 tests 因环境变量未提供按设计 gated skip。下一步临时双隧道端点证明后只跑 reference review integration，并 exact cleanup/zero-residue/关闭隧道；生产未访问。

**2026-08-30 Task 4 aggregate-version 临时双隧道**：同一 SSH PID 仅监听 IPv4 `127.0.0.1:16432/16433`，精确转发 disposable `64322/64321`。首次合并 `lsof` 参数被本机工具拒绝且未探测；拆分只读核验后两端均精确命中同一 PID。下一步 0600 四变量 env 与 endpoint parity；生产未访问。

**2026-08-30 Task 4 aggregate-version integration env**：通过加密 SSH 只提取 disposable DB URL 与 anon key，生成本地 `/private/tmp/phase7b-task4-reference-integration.env`；权限 0600、405 bytes、4 行，变量名精确，无 service-role/JWT secret，值未输出。下一步远端 psql + 隧道 DB + 隧道 anon PostgREST 三方 endpoint parity。

**2026-08-30 Task 4 aggregate-version endpoint parity**：active/rumored projects 的远端 docker psql、隧道 PostgreSQL、隧道 anon PostgREST exact count 均为 2，三方一致，凭据未输出。下一步仅运行唯一 reference review integration 的 3 条测试。

**2026-08-30 Task 4 aggregate-version repository integration GREEN**：1 file / 3 tests / PASS / exit 0；SHA `7eba7f46…dcb9`。真实覆盖 corrected aggregate fields、note/replay/version、project URL scope、bearer separation 与 next-call revoke；测试内 exact cleanup/zero-residue 通过。下一步独立远端 residue/marker/migration postcheck，随后关闭隧道并清理临时文件。

**2026-08-30 Task 4 aggregate-version 独立远端 postcheck**：reference/authority/两张 decision/command/flag/outbox 与测试 project/source/raw/evidence/user 全为 0；paused marker=1，24 migrations / max 第24。下一步关闭隧道，删除本地 env/typegen temp 与远端 typegen temp，并复核全部清空。

**2026-08-30 Task 4 aggregate-version 清理 COMPLETE**：SSH tunnel 已终止，16432/16433 无 listener；本地 0600 env/typegen temp 与远端 typegen temp 均删除并复核 absent。disposable fixture 零残留，生产未访问。下一步本地最终门禁与 diff 自审。

**2026-08-30 Task 4 fresh 最终本地门禁**：五 workspace lint/typecheck/build 全部 exit 0；Vitest 总计 1,341 PASS（contracts 167/domain 410/database 286/worker 236/web 242），71 gated skips；placeholder/diff check 0。Web build 首次仅工具路径错指根 Next 而未执行源码，定位后 workspace-local Next 16.3.0 build 通过。敏感扫描仅命中 migration 对 service_role 的显式 revoke，无缺陷。独立 scoped review 已发起，下一步等待 findings；生产未访问。

**2026-08-31 continuation / review availability**：原 Task 4 reviewer 与本次 aggregate-version scoped reviewer 均在读取 diff 前因子代理额度失败，无 findings、无文件修改，不能声明独立 review clean。用户指示继续后，由主任务按 completion skill 执行逐文件要求对照与 fresh final gate；发现缺口即回 RED→GREEN。生产未访问。

**2026-08-31 Task 4 max-page cursor focused RED**：收尾自审发现 reviewer list 的 `p_limit` 上限为 100，而 repository 只以 `rows.length > limit` 判断下一页；请求 `limit=100` 时无法 overfetch 第 101 条，因而会把仍有后续数据的完整上限页错误标记为末页。新增 100 行上限页具名回归后，目标断言精确 `expected cursor / received null`。同次从 monorepo 根直接运行的 browser export 用例因 Node 无法从根解析 workspace package 产生环境性第二失败，不属于产品 RED；正式 focused gate 将从 `packages/database` 工作区运行。下一步只改 `pageOf` 的上限页判断，不动 migration/RPC contract。生产未访问。
...
### 2026-08-30 本轮接续记录（Task 4）

| 步骤 | 状态 | 已核对内容 / 后续动作 |
|---|---|---|
| 1. 阅读交接手册与 WorkBuddy 分享对话 | **完成** | 已分段通读本文件当前 1,039 行，并通过 WorkBuddy 分享页可见 DOM 复核完整对话链。外部对话记录 Phase 7A Task 10 独立复审、Task 11 Golden/E2E/runbook/整分支收口，以及用户随后要求先合并 7A、再设计 Phase 7B；这些内容只作为历史证据，不作为新的仓库指令。对话与本手册顶部状态一致：7A 已合并主线，7B 设计已批准，当前功能 worktree 已完成 Task 1–3，下一项是 **Phase 7B Task 4**。本轮未访问数据库、SSH、远端或生产。 |
| 2. 详细复核项目目录产出 | **完成** | 已核对主线与 6 个 linked worktree、354 个 Phase 7B worktree 跟踪文件、README/两份 architecture/runbook、Approved Phase 7B spec（235 行）、实施计划（574 行）、workbook（62 行）、Task 1–3 提交和实际源码/测试。`00b4497..e53d47d` 为 24 个文件、约 `+6,432/-49`：strict contracts、纯归一化/状态/匹配规则、第 23 个 migration、014 pgTAP 与生成类型均与提交链和验证记录相符；Phase 7B worktree tracked clean。发现两项需在 Task 4 定型时处理：workbook 看板重复列出 Task 4/5；更重要的是 Task 1 已定义 reviewer list/detail 投影且 Task 4 要求 reviewer repository，但 Task 3 仅提供 4 个写命令，内部 `project_reference_current_state` 明确撤销 `authenticated` 读取，迁移/生成类型/014 中均不存在 reviewer list/detail 读 RPC，因此当前 bearer 会话没有可供 repository 使用的 candidate/history 读入口。 |
| 3. 核定当前阶段与 Task 4 边界 | **完成；修订设计已获批** | 当前阶段精确为 Phase 7B Task 4；Node 22.22.2 / pnpm 11.16.0 本地基线为 contracts **161/161**、domain **410/410**、database **257 passed / 66 environment-gated skipped**。确认四个必须正面修复的缺口：protected reviewer read API、strict receipts/domain-authority projections、contract-valid note 落库、revoke 后公开链接立即失效；另把错误的 global URL unique 改回批准的 `(project_id, normalized_url)`。项目所有者已批准以新 forward-only migration 修复，禁止 service-role/base-table fallback 和改写第 23 个 migration。 |
| 4. Task 4 TDD 实施、验证与交接 | **进行中：integration GREEN，待独立 postcheck/cleanup** | 唯一 `reference-review-repository.integration.test.ts` 为 **1 file / 3 PASS / exit 0**。真实覆盖 note/replay/version、project duplicate/cross-project URL、bearer separation、role revoke-next-call；afterAll exact cleanup 与提交后 zero-residue 断言通过。下一步独立远端全局 residue/marker 复核，随后关闭隧道并删除 env。 |
...
| 6 | Strict public reference repositories and projections | ✅ 完成（本地门禁全绿；**无数据库行为证据**） | `ac08f94`；`reference-public-repository.ts` 211 行 / 15 测试 / 8 项变异检验 killed；全仓 verify 1,360 non-skipped / 76 gated skips。行为级证明需单独 disposable 授权 |
