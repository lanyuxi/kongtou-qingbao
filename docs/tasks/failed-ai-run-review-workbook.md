...
| 5 | REVIEW | Reviewer session and browser API clients | Initial modules were absent; whitespace tokens failed 2 named cases; fix round 1 bundle lacked both public markers; fix round 2 cleanup regression deleted a pre-existing sibling sentinel. | Fix round 2: isolated real client build preserves its sibling sentinel; Web 98/98, exact public-only bundle inspection, lint/typecheck/build, and Node 22 root verify (768 tests) passed. | `a5d27e3`, `89d06e8`, `test(web): isolate reviewer bundle build` |
