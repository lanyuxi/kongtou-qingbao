...
| 7 | DONE | 注册 DeFiLlama 源（已授权） | 所有者授权后执行。**预检**：3 个项目均 active（ethereum/solana/arbitrum），`verified_by` 用**所有者真实 profile** `7fb6df98-…`（不是 seed fixture profile）。**回滚空跑通过后真实写入**：3 条 `sources`（`source_type='chain_explorer'`、`collectable=true`）+ 3 条 `project_sources`（`authority_domains=['api.llama.fi']`、`is_official=false`、`verified_at`/`verified_by` 非空）+ 3 条 `source_collection_schedules`（43200s / 12h、`enabled=true`、`enablement_origin='manual'`、`next_run_at` 错开 5 分钟）。**`is_source_collection_eligible` 三条全部 = true。** |
| 8 | DONE | 真实载荷适配器验证 | 抓取三个链的**真实** DeFiLlama 响应（120762 / 73767 / 68527 字节，`application/json`，均 < 2MiB 上限），用**注册表按 DB 里的 canonical_url** 选适配器并解析：三条均产出恰好 1 条 entry，`entryUrl=null`、稳定键 `defillama:chain-tvl:<chain>`、摘要含确定性分组数值（如 `49,288,764,438 USD as of 2026-09-11`）、长度 67–70（远低于 2000 上限）。**LIVE ADAPTER CHECK: PASS**。 |
| 9 | DONE | pgTAP（已授权安装扩展） | 安装 `pgtap` 到 `extensions` schema 后运行。**`023_phase_16_json_api_collection` PASS 3/3**（本 Phase 的验收测试）；`022_phase_10_execution` PASS 73/73。**全量 23 文件：pass=7 / fail=12 / abort=3 / skip=1，ok=1513 / notOk=43** —— 失败**全部是既有的「云生产 vs 全新 disposable」差异，与 Phase 16 无关**：①四个 worker 角色 `rolcanlogin=true`（Phase 12 为走 pooler 而开启；`rolsuper`/`rolbypassrls`/`rolcreatedb`/`rolcreaterole` 均为 false，**最小权限未破**）→ 008 的「non-login」断言失败；②Phase 14 有意做了 schema 级函数授权 → 010/011/021 的「仅 X 可执行」断言失败；③生产是真实数据而非 seed fixture → 007/006 等断言失败；④3 个文件含 psql 元命令（`\set` 等）无法经协议执行、若干重文件触及云端 statement timeout（runner 已加 `statement_timeout=0` 与逐文件 rollback 重置）。**结论：该套件按设计面向全新 disposable 栈，在云生产上不可能全绿**；本 Phase 的判据是 023 通过 + 回滚式行为探针 4/4 + 真实载荷验证通过。 |
| 10 | DONE | push 到 GitHub（已授权） | `GIT_SSH_COMMAND="ssh -i ~/.ssh/github_ed25519 -o IdentitiesOnly=yes" git push origin HEAD:main` → `91368ca..aa4f5b4`，随后 `origin/main...HEAD` 为 `0 0`（完全同步）。 |
| 11 | BLOCKED | Vercel 部署 | **本机无任何 Vercel 凭据**：`~/.vercel/auth.json` 不存在、`VERCEL_TOKEN` 未设、无 `.vercel` link、记忆里也没有 token；且 `npx vercel` 在此环境被沙箱拒绝。**故未部署。** ⚠️ **影响**：线上构建仍是 2026-09-03 版本，其 `parseCollectionMediaType` **不接受 `application/json`**，所以已注册的 3 个 JSON 源在下次 cron 只会记为 `unsupported_content_type`（优雅失败、不写脏数据），**必须重新部署后才会真正采集**。需要所有者提供 Vercel token，或在自己的终端执行一次部署。 |
