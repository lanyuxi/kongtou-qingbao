...
| 日期 | 事项 | 备注 |
| 2026-09-09 | Task 7 收口：runbook + 敏感扫描 + verify | ①runbook 新增「Execution: tasks, watchlists, and participation (Phase 10)」章节（在 `## Shutdown` 前，骨架与 7B/8/9 同构：范围铁律→D1–D9 决策表→命令面（9 命令 + 4 读 RPC、上限 500/20/200、默认列表 `默认关注`）→EX2xx 错误码表 13 条→focused 命令与隧道/代理/变量推导→append-only 恢复→上线状态与剩余 gate）。**事实回读迁移与契约对拍后写**（上限常量逐条 grep 自迁移32；错误码取自 `enums.ts`），不凭工作簿记忆。②敏感扫描：执行域密钥词命中仅 7 处，**全在契约层且均为防御本身**（`forbiddenKeyLikeFields` 的正则字面量、注释、`ExecutionForbiddenFieldError` 文案）→ 零真实泄漏。③root `pnpm verify` 全绿（1m30s）；逐包计数 web **623**（33 files）/ database **362**（87 skipped 为无环境集成用例）/ contracts **277**（22 files）。 |
| 2026-09-09 | 生产应用迁移 32/33（授权后执行） | 备份 `/root/backups/prod-pre-phase10-20260909.sql`（9.5M / 103 表）；逐条「回滚事务空跑 exit 0 → 真实应用 + 登记 `schema_migrations`」（生产惯例 `statements='{}'`）。验证：迁移数 **29→31**、`get_my_participation` 签名含 `version bigint`、9 命令 + 4 读 RPC 全在、3 个 profile backfill 出 3 个默认关注列表、user_task_events / execution_command_receipts 空表已建。**遗留事实：生产版本序列跳号**——Phase 9 的 30/31 仍未应用（32/33 已静态核实不依赖它们，可独立应用）；补 30/31 仍受 `GOTRUE_SITE_URL` 公网地址前置约束。 |
| 2026-09-09 | Task 6 DB 侧 disposable 验收完成：022 **73/73**、全套 **1,730/0**、集成 **6/6** | ①迁移33 空跑抓到真实缺陷：`create or replace function` **不能改变返回类型**（投影加列→行类型变），改 `drop function if exists` + `create function`（同事务内）。②022 的 5 条新断言起初全红：我把 `profiles` 的 seed id 当成 project id，而 `submit_set_participation_status` **先校验 project 存在**（EX204）再走乐观锁——真实 project id 是 `...-010/011/012/019`。③集成新增用例同样撞 EX204（`randomUUID()` 不是真 project），改为基于真实 project + 按投影版本构造下一次命令，且对「行是否已存在」稳健。④残留核验：执行域表全 0、projects=4 基线；watchlists 5 条全为默认列表（4 条属集成测试创建后被清理的 auth 用户，为测试用户生命周期的既有副产物，与迁移33无关）。⑤集成变量推导：anon key **不在 kong 容器 env、也没有 .env**，它以值形式在 `/home/kong/kong.yml`（`query_params.apikey ==` 比较里），按 JWT payload `role=anon` 自动挑选；远端 0600 生成→scp 取回→立即删远端→用完删本地。 |
| 2026-09-08 | Task 6 关注列表与参与状态 UI 完成 |
| 2026-09-08 | Task 6 关注列表与参与状态 UI 完成 | ①**修复 Task 5 遗留的 create 命令缺陷**：浏览器端 `createTask` 的草稿校验误用含 `expectedVersion` 的 strict 信封 schema（input 不含该字段→必然 parse 失败→任何创建都返回 `execution_command_invalid` 且 fetch 零调用）；该路径此前无直接测试覆盖（所有测试都 mock 了 client 接口）。修法：草稿 schema omit 信封两字段，client 构造完整命令时补 `expectedVersion: 1`（创建无既有版本，数据库回执也落 1）；`createWatchlist` 同构。②**发现并修复「二次参与设置永远版本冲突」的设计缺口**：`submit_set_participation_status` 是真乐观锁（fresh row 要求 1、existing row 要求匹配 version），但 `get_my_participation` 投影**不返回 version** → UI 拿不到版本，设置过一次后永远 EX209。forward-only 修复 = **迁移 33**（`create or replace` 只替换读函数，补 `"version" bigint` 列，不动迁移 32 文件）+ 契约投影 `userProjectParticipationRowSchema` 补 `version` + pgTAP 022 补 5 断言（fresh row version=1 / 读携带 version / 从投影版本构造第二次更新 / version 递增 / 陈旧版本 EX209，plan 68→**73**）+ database 集成测试补「二次设置往返 + 陈旧版本被拒」用例（5→6，**执行待授权**）。③UI 三件：`watchlist-manager`（默认列表徽章 + 默认列表无删除入口 + 重命名/删除/查看项目/移出 + 重名与上限按 conflict 码展示）、`project-execution-panel` + `ProjectExecutionSection` 客户端容器（项目详情页是 server component 不能持 runtime hook；「未参与」=`get_my_participation` 空行=仓储 `project_not_found`，按 null 参与渲染新表单而非报错）、`/watchlists` 页面；`execution-submission.ts` 共享提交状态机（conflict 码集合单份声明，不与 task-manager 漂移）。④**测试新增**（api-client 15 / watchlist UI 10 / panel 13 / 契约投影 1 / 集成 1），vitest 实测：web **623 passed**（33 files）、contracts **277 passed**、database 仓储 **14 passed**；root verify 全绿（lint/typecheck/test/build/check-placeholders）；**变异 24/24 killed**（8 api-client + 8 watchlist + 8 panel）。⑤**build 才能抓到的坑再现**：两个组件文件漏 `'use client'`——vitest（不做模块图分析）与 tsc 都绿，`next build` 报 Ecmascript error（被 server component 导入后 hooks 导入在 server 图失败）；Task 4 的「只有 build 能抓」教训同族。⑥测试侧两处自纠：契约信封要求 `requestId` 是 **UUID**（夹具用 `'request-1'` 导致 15 用例全落 fallback）；React `Headers` 实例与 vitest `toMatchObject` 不兼容（断言前先 `Object.fromEntries`）。**DB 侧待授权项：disposable 应用迁移 33 + 重跑 022（73 断言）+ 集成测试 6/6。** |
