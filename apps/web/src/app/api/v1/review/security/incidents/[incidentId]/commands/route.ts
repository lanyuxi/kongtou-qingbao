...
import { createSecurityIncidentCommandHandler } from '../../../../../../../../lib/security-review-handlers.js';
import { createServerSupabaseClient } from '../../../../../../../../lib/supabase-server.js';
let handler: ReturnType<typeof createSecurityIncidentCommandHandler> | undefined;
function getHandler() { if (handler !== undefined) return handler; const environment = parsePublicEnvironment(); handler = createSecurityIncidentCommandHandler({ auth: createAuthenticatedUserVerifier(environment), reviews: createSecurityReviewRepository({ url: environment.supabaseUrl, anonKey: environment.supabaseAnonKey }), publicSecurity: createSecurityPublicRepository(createServerSupabaseClient()), ids: { generate: () => crypto.randomUUID() } }); return handler; }
export async function POST(request: Request, context: { params: Promise<{ incidentId: string }> }) { return getHandler()(request, context); }
