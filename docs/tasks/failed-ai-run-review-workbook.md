...
| 6 | REVIEW | Sign-in, list, detail, history, and decision UI | Focused suite first failed before collection because the review UI modules did not exist; production build then reproduced build-time runtime creation as a Zod env failure. | Focused 16/16; Web 114/114; lint/typecheck/build; Node 22 root verify 784 tests; unsafe-field and incompatible-reason mutations each caused named failures before exact restoration. | `feat(web): add failed AI run review interface` |
