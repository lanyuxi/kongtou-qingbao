...
| 4 | REVIEW | Authenticated review BFF and routes | Focused Web suite failed before collection because `../lib/failed-ai-run-review-handlers.js` did not exist; fix-round leak mutation then produced 3 named failures across list/detail/decision. | Fix round 1: focused handler 52/52; Web package 69/69; exact canonical error envelopes, auth fail-closed, wrong-operation fallbacks, secret exclusions, lint, typecheck, build, Node 22 root verify (739 tests), and diff checks passed. | `d5f6f42`, `test(web): harden failed AI review API errors` |
| 5 | REVIEW | Reviewer session and browser API clients | Initial modules were absent; whitespace tokens failed 2 named cases; fix round production build succeeded but the real client chunk lacked both public markers under indirect `process.env` aliasing. | Fix round: client-bundle/session 11/11, Web 98/98, exact public-only bundle inspection, lint/typecheck/build, and Node 22 root verify (768 tests) passed. | `a5d27e3`, `fix(web): inline reviewer public env` |
