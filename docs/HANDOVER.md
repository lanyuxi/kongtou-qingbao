...
> 最近更新：2026-08-29（**Phase 7A Task 1–11 全部完成，整分支 review clean**：Task 10 复审 0C/3I/3M 已闭环并 APPROVED；Task 11 七步全部完成——Step 3 经你授权后执行 disposable 矩阵（reset exit 0、full pgTAP **13 files / 1,338 PASS**、集成矩阵 **9 files / 66 tests PASS**、清理后回到 seed 基线零残留），Step 6 复审 0C/2I/4M 亦全部闭环。最终 `pnpm verify` exit 0：contracts 133 + domain 376 + database 257 + worker 236 + web 242 = **1,244 non-skipped / 68 gated skips**。生产未访问、未应用迁移；隧道已关闭、凭据已删）
> 权威规范：仓库根目录 `AGENTS.md`（产品规则与工程约束的唯一事实来源，本手册不重复其内容，只补充现状与经验）
>
> **当前一句话状态（2026-08-29 Phase 7A 全部分支完成）**：Phase 7A 在 linked worktree `.worktrees/phase-7a-security-ledger` / 分支 `codex/phase-7a-security-ledger` 上 **Task 1–11 全部完成并通过独立复审**，HEAD 为 `d38f9af`（+ 本条文档提交）。全栈纵切片已齐备：strict contracts → 纯 domain 规则 → 追加式 Security Ledger 迁移/受保护命令/RLS → bearer-scoped repository → extraction/Promotion 路由 → collection/scoring 门禁 → 公共安全投影 → BFF/浏览器客户端 → reviewer UI → 公共呈现 → Golden Dataset/runbook/disposable 验收。分支仍**未合并主线**；生产仍为 19 个已应用迁移，Phase 7A 迁移继续按 production unapplied 处理，任何 rollout 仍须实时 preflight、备份、Auth/reviewer readiness 与显式授权。
