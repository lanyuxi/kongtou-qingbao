...
| 6 | DONE | Sign-in, list, detail, history, and decision UI | Focused suite first failed before collection because the review UI modules did not exist; fix round 2 made opaque branches independent; fix round 3 exposed the every-segment-8-character compact-token gap. | Fix round 3: focused 28/28; Web 126/126; lint/typecheck/build; Node 22 root verify 796 non-skipped tests; short/empty compact JWS segments are hidden from decoded-header evidence while semantic/natural dotted identifiers remain visible; scoped re-review accepted. | `85be7dc`, `f6a3955`, `bc1f401`, `71a80b7` |
| 7 | DONE | End-to-end regression and authorization matrix | Raw-detail parser bypass failed the named HTTP sanitization test; real revoked fixture first failed `user_roles_revoked_after_granted`; removing all three revoked-role predicates failed exactly the named AR104 integration test; fix round 1 registration-after-confirmation mutation failed the exact partial-cleanup test. | Original disposable focused 6/6, full integration 40/40, and pgTAP 1031/1031 passed; fix round 1 Node 22 root verify 798 non-skipped tests; WorkBuddy acceptance re-verification on disposable API 64321 / DB 64322: focused integration 6/6 (Node 22.22.2, 4.25s), fixture unit 5/5, full `test:integration` 7 files / 40 tests passed; scoped re-review found no Critical/Important breakage (changes confined to test files plus docs; all three `signUpFixture` call sites register IDs; teardown keeps marker verification and exact-ID deletion). | `b2f104b`, `e9b3e52`; acceptance commit pending |
| 8 | DONE | Runbook, security review, and repository gates | Documentation RED: the runbook previously had no failed-run review operator section; the secret-scan hit list required manual classification before acceptance. | Runbook section added (provisioning, sign-in, focused tests, migration safety, scope exclusions); every scan hit classified as fixture/prohibition/server-only bearer plumbing/schema column/ACL revocation with no actual secret; Node 22 root `pnpm verify` exit 0 with 798 non-skipped tests; disposable reset exit 0 and full pgTAP Files=11 / Tests=1031 PASS; `git diff --check` clean; local and remote artifact SHA-256 identical. | acceptance commit pending |
| 9 | BLOCKED | Controlled production rollout checkpoint | Read-only preflight must find a user-approved human reviewer, reachable production Auth, and a backup posture before any mutation. | Preflight (read-only): production ledger 19 migrations, max `20260820000300`; `ai_run_review_decisions` absent (not applied); the only two `auth.users` rows are `@example.invalid` operational fixtures (`fixture-reviewer` and Phase 6A `ops-reviewer`) — no human reviewer account; production Auth API `/auth/v1/health` returns 503 (no auth container in `docker ps`; REST 200); no periodic DB backup job, only historical manual backup directories. Stopped at `PRODUCTION_READY — REVIEWER_ACCOUNT_REQUIRED`; no production mutation, migration application, or account provisioning occurred. | docs only |
...
| Phase | Evidence |
| --- | --- |
| Review finding | The prior long opaque fixture also contained `data:`, provider/raw/output, and secret markers. Removing `longOpaqueValue` therefore stayed green, while dot-separated JWTs and 24–47 character mixed-class machine tokens could render. |
| Independent RED | A realistic JWT in a decision note plus a different three-segment value in `modelId` failed the named JWT test with no other sensitive predicate present. Separate 24- and 27-character mixed-class values in a note and `pipelineVersion` failed the named short-machine-token test. |
| Minimal policy change | `displayReviewValue` now recognizes three base64url-like segments and short no-whitespace token-shaped values that combine uppercase, lowercase, digits, short alphabetic runs, and a high unique-character ratio. The existing 48+ opaque branch remains unchanged. |
| False-positive guard | A page fixture keeps required UUIDs, `provider_error`, ordinary note/source/model text, and `Project-Release-2026-Alpha` visible. This prevents treating every 24-character natural identifier as a credential. |
| Mutation sensitivity | Removing only the JWT branch failed exactly `hides JWT-shaped values independently in notes and allowed metadata`. Bypassing only the short-token branch failed exactly `hides short mixed-class machine tokens independently in notes and allowed metadata`. Both branches were restored before GREEN. |
| Restored gates | Focused 26/26; Web 7 files / 124 tests; lint/typecheck/build/placeholders; Node 22.22.2 root `pnpm verify` 794 non-skipped tests. No deployment, external call, dependency, credential, or production access occurred. |

## Task 6 fix round 3

| Phase | Evidence |
| --- | --- |
| Review finding | The length-only JWT regex required every segment to contain at least eight characters. An HS256 compact token whose `{}` payload is `e30`, detached compact JWS with an empty payload, and unsecured compact JWT with an empty signature bypassed it. |
| Independent RED | Page fixtures placed two cryptographically valid HS256 compact values with `e30` payloads in a decision note and `modelId`, then detached and unsecured compact values in a note and `schemaVersion`. Both named tests failed under the fix-round-2 policy without keyword, scheme, control, or machine-payload triggers. |
| Structural policy | Compact credential detection now requires exactly three base64url segments, permits empty payload/signature segments, safely decodes the protected header, and hides only when it parses to a JSON object with a non-empty string `alg`. Cryptographic verification is intentionally not attempted because the reviewer UI has no signing material. |
| False-positive guard | The arbitrary all-segments-length predicate was removed. `1.2.3`, `release.candidate.stable`, and `releasecandidate.buildcandidate.stablecandidate` remain visible alongside UUIDs, enums, and ordinary text. |
| Mutation sensitivity | Removing only `resemblesCompactJws(value)` failed exactly `hides compact HS256 tokens with short payloads in notes and allowed metadata` and `hides detached-payload and unsecured compact JWS values`. The predicate was restored before GREEN. |
| Restored gates | Focused 28/28; Web 7 files / 126 tests; lint/typecheck/build/placeholders; Node 22.22.2 root `pnpm verify` 796 non-skipped tests. No deployment, external call, dependency, credential, or production access occurred. |

## Task 7 execution log

| Phase | Command/check | Expected result | Actual result |
| --- | --- | --- | --- |
| HTTP sensitivity RED | Temporarily bypass strict detail response parsing and run the malformed-repository-result test | A raw repository object must never reach JSON | The named `sanitizes a malformed repository detail instead of returning raw failure content` test failed with HTTP 200 versus expected 500. Strict parsing was restored before GREEN. |
| First real fixture RED | Fresh-reset the marker-verified disposable stack and run the focused repository integration | Exact randomized Auth fixtures satisfy real schema constraints | Suite setup failed at `user_roles_revoked_after_granted` because the revoked role used equal grant/revoke times. Product code was untouched; `e9b3e52` now grants two hours before revoking one hour before test time. |
| Focused real GREEN | Run only `failed-ai-run-review-repository.integration.test.ts` through the disposable API 64321 / DB 64322 tunnels | Complete six-test reviewer workflow, denials, ACL matrix, and both races pass | 1 file / 6 tests passed in 4.60s. Three reviewable statuses were returned; succeeded/skipped controls were excluded; the two-version decision/receipt/outbox history matched exactly. |
| Full repository integration | Run package `test:integration` after a fresh disposable reset | Existing repository boundaries remain green | 7 files / 40 tests passed, explicit exit 0, in 80.56s. Cleanup verified the disposable marker and removed only exact randomized fixtures. |
| Role-denial sensitivity | Remove all three `revoked_at` predicates only from the disposable migration copy | The revoked-reviewer behavior regression fails by name | Mutated SHA-256 `1a1b153b8e866631b64453808222f77cf3967a991885679624614bb3a1c6e29c`; 6 tests ran, exactly 1 failed: `returns AR104 for every ordinary-user and revoked-reviewer RPC operation`; the other 5 passed. |
| Restore | Restore the committed migration, reset, and rerun focused integration | Artifact identity and focused behavior return to GREEN | Migration restored to SHA-256 `5150c830cf2cec7850c179f0fbf7254b6c658287fa96789420c6cb75dbb755ae`; reset exited 0; focused 6/6 passed in 4.59s. |
| Database and root gates | Run full pgTAP plus required-runtime root verification | All repository gates pass on the committed artifacts | pgTAP 11 files / 1031 tests passed. Node 22.22.2 / pnpm 11.16.0 `pnpm verify` passed 797 non-skipped tests; lint, typecheck, build, placeholders, and diff check passed. Production 54321/54322 was untouched. |

Implementation notes: the integration uses real Supabase Auth sessions for an active reviewer, ordinary active user, and revoked reviewer. The active reviewer lists only the three failed statuses, reads an exact safe detail, appends `needs_investigation`, replays without duplication, and appends `dismiss` at the next version. The final snapshot contains exactly two append-only decisions, two receipts, and two safe outbox events with exact linkage. Anonymous, ordinary, revoked, authenticated-table, worker, collector, and service-role boundaries are separately executable. The malformed HTTP result regression proves raw error detail, source text, bearer tokens, and passwords collapse to the canonical sanitized 500 envelope.

## Task 7 fix round 1

| Phase | Evidence |
| --- | --- |
| Review finding | `signUpFixture()` exposed the user ID to outer cleanup state only after confirmation SQL/sign-in completed. A failure after Auth created the user but before the helper returned left `afterAll` without that exact ID. Task 7 had also replaced the tested two-user cleanup helper with an untested inline three-user filter. |
| Tests-first RED | The helper table now covers reviewer-only, reviewer plus ordinary, all three, and none. Before implementation, the all-three case omitted the revoked ID and the new registration helper was absent: 5 focused tests ran, 2 failed. |
| Ordering sensitivity RED | Moving `registerUserId` after the injected post-signup completion failed exactly `registers the Auth user before post-signup confirmation can fail`: received `[]` instead of the reviewer ID. The mutation was restored. |
| Minimal fix | A shared `Set` registers each returned Auth user ID immediately before confirmation SQL or sign-in. Teardown uses the extended `presentFixtureUserIds(...createdUserIds)` helper, preserving deduplication, empty filtering, marker verification, and exact-ID deletion. |
| Local and required-runtime GREEN | Focused helper 5/5; database 159 passed / 40 integration skipped; Web handler 53/53; database lint/typecheck. Node 22.22.2 / pnpm 11.16.0 root `pnpm verify` passed 798 non-skipped tests with lint/typecheck/build/placeholders green. |
| Pending acceptance | Automatic approval rejected the SSH credential read for disposable API 64321 / DB 64322 because the account usage limit was reached. Per the rejection, no retry or workaround occurred. The post-fix focused real integration and scoped re-review are still required; Task 7 remains REVIEW. Production was untouched. |

## Task 7 acceptance (WorkBuddy, after Codex quota exhaustion)

| Phase | Evidence |
| --- | --- |
| Handover scope | Read HANDOVER, 6B spec/plan/workbook/progress, and the uncommitted diff. The fix round 1 diff is confined to three test files plus docs; no product, migration, or generated-type change. |
| Credential path | A remote derivation script with double `governance-test` container-name allowlist derived the four disposable integration variables from the disposable db/kong containers only; the script printed counts, values returned over the SSH channel into local `/tmp/airdrop-6b-test.env` (umask 077 / chmod 600), remote copy deleted. Tunnels limited to `16432 -> 64322` and `16433 -> 64321`; `http://127.0.0.1:16433/auth/v1/health` returned 200. Production ports 54321/54322 have no tunnel and were not accessed. |
| Post-fix focused re-verification | `env -u NODE_OPTIONS pnpm exec vitest run --no-file-parallelism src/tests/failed-ai-run-review-repository.integration.test.ts` (Node 22.22.2): 1 file / 6 tests passed in 4.05s (4.25s wall): full real reviewer workflow, AR104 ordinary/revoked, anonymous/table denials, ACL matrix, and both independent-client races all green. |
| Fixture unit and full integration | `failed-ai-run-review-integration-fixture.test.ts`: 1 file / 5 tests passed; package `test:integration`: 7 files / 40 tests passed in 74.19s with marker-verified exact-fixture cleanup. |
| Scoped re-review | Verified all three `signUpFixture` call sites pass the registration callback; `completeRegisteredFixtureUser` registers the ID before any post-signup step; teardown derives deduplicated exact IDs through the tested helper inside marker-verified `removeExactFixtures`; ordering sensitivity test `registers the Auth user before post-signup confirmation can fail` passes; change set contains no production/migration/generated-type/dependency edits. No Critical/Important breakage. Task 7 accepted DONE. |

## Task 8 execution log

| Phase | Command/check | Expected result | Actual result |
| --- | --- | --- | --- |
| Runbook operator flow | Add "Failed AI run review (Phase 6B)" section to `docs/runbooks/local-development.md` | Documents provisioning, sign-in, focused tests, migration safety, and scope exclusions without credentials | Section added: flat-SQL reviewer provisioning (no `DO $$` over SSH, no guessed identity), `user_roles.revoked_at` immediate-revocation semantics, `/review/sign-in` flow, focused test commands, disposable-only integration variables, forward-only additive migration safety, and the explicit exclusion of failed-run retry. The UI never creates accounts or roles. |
| Secret and unsafe-field review | `rg` for `service_role|private_key|seed phrase|mnemonic|password=|Bearer |error_detail|raw_text|AI_MODEL_API_KEY` across `apps/web`, `packages/contracts`, `packages/database`, and the 6B migration | Every match manually classified; no actual secret may appear | 60+ matches all classified: test fixtures that prove sanitization, prohibition/deny lists (`safe-review-text.ts` keyword blocklist), server-only bearer plumbing (`authenticated-user.ts`, handlers, repository request headers), existing schema columns (`raw_items.raw_text`, `ai_runs.error_detail`, generated types), ACL revocation lists in the migration (revoke from all roles including `service_role`; grants only `execute` to `authenticated`, re-checked inside each security-definer function). No real secret, key, or token appears. |
| Artifact identity | `shasum -a 256` locally and remotely for the 6B migration and 011 test | Local committed bytes equal the disposable-stack executed bytes | Migration `5150c830cf2cec7850c179f0fbf7254b6c658287fa96789420c6cb75dbb755ae`; test `e9158648ea970d930a1fd9cd240d67d4d6e380b41c00d0c32754563368cda7e1`; identical on both sides. |
| Full database gate | Remote disposable `supabase db reset --yes` then `supabase test db` | Reset exit 0; all pgTAP files pass | `RESET_EXIT=0`; Files=11 / Tests=1031 / Result: PASS on `airdrop-intelligence-governance-test` only. Production untouched. |
| Full repository gate | Node 22.22.2 / pnpm 11.16.0 `pnpm verify` | Lint, typecheck, test, build, placeholders exit 0 | `VERIFY_EXIT=0`; contracts 78, domain 222, database 159 passed / 40 skipped, worker 212 passed / 2 skipped, Web 127 (798 non-skipped total); build lists the three review API routes and the three review pages. |
| Patch hygiene | `git diff --check` and `git diff HEAD --check` | No whitespace errors | Both exit 0 (`DIFF_CHECK_OK`). |

## Task 9 execution log — PRODUCTION_READY — REVIEWER_ACCOUNT_REQUIRED

| Phase | Command/check | Expected result | Actual result |
| --- | --- | --- | --- |
| Migration ledger | Read-only `select count(*), max(version) from supabase_migrations.schema_migrations` on production | 19 applied migrations before rollout | 19 rows, max `20260820000300`; the 6B migration is unapplied. |
| 6B objects | Read-only existence check for `public.ai_run_review_decisions` | Absent until rollout | `false` — production schema untouched by Phase 6B. |
| Human reviewer account | Read-only `user_roles` plus `auth.users` email-domain scan | A user-approved human reviewer with an active reviewer-class grant | **Absent.** Both `auth.users` rows use `@example.invalid`: `90000000-…-01` (`fixture-reviewer`, no active role) and `a0000000-…-01` (Phase 6A `ops-reviewer`, active). Operational fixtures must not become human logins; no account with guessed identity was created. |
| Auth/API reachability | `curl` against production Kong from the host | Healthy Auth for reviewer sign-in | `/auth/v1/health` returned **503**; no `supabase_auth_airdrop-intelligence-os` container in `docker ps`. `/rest/v1/` returned 200. The reviewer UI cannot sign in until the production Auth service exists. |
| Backup/recovery posture | Host `crontab -l` and backup-directory scan | A documented recovery posture before the first production migration | No cron-based DB backup; only historical manual backup directories from earlier phases (`task4-phase2-backup`, `task5-rsync-layout-backup`, `task9-concurrency-rsync-backup`). A pre-rollout backup procedure must be established and exercised. |
| Stop decision | Plan Task 9 Step 2 | Halt without mutation when human reviewer provisioning is absent | **`PRODUCTION_READY — REVIEWER_ACCOUNT_REQUIRED`** recorded. No production migration, role, account, data, or configuration change occurred. Rollout Steps 3–5 remain unexecuted pending: (1) project-owner provisioning or identification of a human reviewer Auth account with an active reviewer-class role; (2) restoring/starting the production Auth service and confirming `/auth/v1/health` 200; (3) a tested pre-rollout backup; then explicit rollout authority. |
