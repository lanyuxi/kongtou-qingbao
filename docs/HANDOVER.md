...
**下一步工作（按序）**：
1. **Task 7 公开页**——详情页教程卡 + `/tutorials/[id]`（消费 `public_project_tutorials` / `public_tutorial_detail`，无裸 URL）。
2. Task 8 AI 生成器——worker 编排 tick 把信号抽取产物转成 `tutorial_candidates`（AI 输出仅到候选，永不直写 canonical）。
3. Task 9 Golden Dataset——教程生成边界用例（提示注入 / 越权字段 / 幻觉 referenceId / 越权枚举）。
4. Task 10 收口——runbook Phase 8 章节、敏感扫描、独立复审、最终 disposable 矩阵与台账。
5. **独立事项（Phase 8 完成后）**：①迁移 26 上生产（需显式授权 + 备份演练沿用 §5 规程）；②生产 web 应用部署（当前无持久 web 进程，nginx 仅静态站）；③Phase 8 分支合并回主线（沿用 7B 双父 merge 惯例）。
