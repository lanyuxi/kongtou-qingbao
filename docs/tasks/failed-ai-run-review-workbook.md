...
| 6 | REVIEW | Sign-in, list, detail, history, and decision UI | Focused suite first failed before collection because the review UI modules did not exist; fix round 2 made opaque branches independent; fix round 3 exposed the every-segment-8-character compact-token gap. | Fix round 3: focused 28/28; Web 126/126; lint/typecheck/build; Node 22 root verify 796 non-skipped tests; short/empty compact JWS segments are hidden from decoded-header evidence while semantic/natural dotted identifiers remain visible. | `85be7dc`, `f6a3955`, `bc1f401`, `fix(web): detect compact JWS credentials` |
