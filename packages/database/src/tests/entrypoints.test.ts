...
describe('@airdrop/database entry points', () => {
  ...
  it('rejects the tutorial review package export under browser module resolution', () => {
    const result = spawnSync(
      process.execPath,
      ['--conditions=browser', '--import=tsx', '--eval', "import('@airdrop/database/tutorial-review')"],
      { encoding: 'utf8' },
    );

    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      '@airdrop/database/tutorial-review is unavailable in browser code.',
    );
    expect(result.stderr).not.toMatch(/supabase|bearer|token|service.role/i);
  });
});
