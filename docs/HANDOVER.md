...
- **无需授权、可随时做的后续项**（复审 N-2/N-3，均为 Minor 且 fail-closed）：①`referenceUrlSchema` 在 `commands.ts:32` 与 `projections.ts:16` **各有一份重复声明而非共用**，且 `projections.ts` 那份缺控制字符 refine，建议改为从 `commands.js` 导入以消除重复；②SQL 的 `normalize_reference_url_v1` 缺长度检查，**可达但 fail-closed**（空路径归一化补 `/` 会 +1 字符，2048 输入可存成 2049 并在读取时抛错影响整页引用列表），要彻底消除需新增 forward-only 迁移，属 Task 3 冻结迁移之外的独立决策。
