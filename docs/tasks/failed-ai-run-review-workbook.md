...
| 6 | REVIEW | Sign-in, list, detail, history, and decision UI | Focused suite first failed before collection because the review UI modules did not exist; fix round 1 added seven behavior/security regressions; fix round 2 proved the previous opaque fixture did not exercise `longOpaqueValue`. | Fix round 2: focused 26/26; Web 124/124; lint/typecheck/build; Node 22 root verify 794 non-skipped tests; independent JWT and 24–47 character machine-token branches are mutation-sensitive while safe lookalikes remain visible. | `85be7dc`, `f6a3955`, `fix(web): detect opaque review credentials` |
