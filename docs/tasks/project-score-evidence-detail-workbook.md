...
## Final whole-branch review fix — complete citation pagination

| Phase | Command or check | Result |
| --- | --- | --- |
| Review finding / preflight | Final whole-branch review at exact HEAD `e95e79495e724f2f771025bc1101a2c70caf7c25`; inspect citation repository and `supabase/config.toml` | One Important confirmed: Data API `max_rows = 1000`, but the citation repository issued one uncounted/unpaged select while score/signal/Evidence links have no 1000-row bound. A successful response could silently omit later or conflicting Evidence. Worktree was clean; factor reads remain bounded by the nine factor codes and are explicitly out of scope. |
| Unit RED | Extend the fake to emulate the 1000-row Data API cap and add 1001-row completeness plus count-null/count-change/short-page/duplicate/second-page-error tests; run exact `project-score-evidence.test.ts` | Exit 1 as required: 6 new tests failed and 14 existing tests passed. The primary assertion received 1000 instead of 1001; the final conflicting citation was absent. All count/page integrity cases and the later-page error were incorrectly resolved/accepted by the current one-request implementation. |
| Focused GREEN | Implement 1000-row pages with exact count, required stable orders, range, per-page dual filters and strict parsing; rerun exact unit file | Exit 0: 1 file / 20 tests passed. All 1001 citations including the final conflict are returned; null/changed counts, short pages, duplicates, and later-page PostgREST errors fail safely through `ProjectEvidenceCitationQueryError`. |
| First package-gate iteration | Run database lint and typecheck | Lint exit 0. Typecheck exit 2 on one test-helper annotation: `createCitationRows` declared `typeof citationRow[]`, incorrectly narrowing dynamic Evidence IDs to the base fixture's single literal ID. Production code had no type error; fix is to use the existing shared public citation row type. |
| Corrected package GREEN | Reuse `PublicProjectEvidenceCitationRow` for the dynamic fixture, then rerun focused unit, database lint, and typecheck | Unit 1 file / 20 tests passed; lint and typecheck both exit 0. |
| Count-check sensitivity RED | Temporarily remove only the later-page expected-count equality check, run the exact changed-count named test, then restore immediately | Exit 1 as required: 1 failed / 19 skipped; the promise incorrectly resolved. This proves the regression test detects removal of cross-page count verification. The mutation was restored and is not part of the intended diff. |
| Restoration package GREEN | After restoring the guard, rerun exact citation unit, database lint, and database typecheck under Node 22.22.2 / pnpm 11.16.0 | All exit 0; unit 1 file / 20 tests passed. |
| Marker-verified disposable integration | Validate 0600 env shape as exact loopback 16432/16433 without printing values; run full database `test:integration`, whose suites recheck the fixed paused marker before writes | Sandbox-only attempt stopped on socket `EPERM` before writes. Approved identical command passed 7 files / 44 tests, exit 0 in 106.29s. Production and 54321/54322 were not accessed. |
